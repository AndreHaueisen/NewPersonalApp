...
class SearchSpecificPersonalActivity : AppCompatActivity(), FirebaseImageCatcher.FirebaseCallback, ClassDurationDialogFragment.DurationCallBack {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_search_specific_personal)

        JodaTimeAndroid.init(this)

        setDefaultProfileImage()

        val toolbar = findViewById(R.id.search_specific_personal_toolbar) as (Toolbar)
        setSupportActionBar(toolbar)
        supportActionBar!!.setDisplayShowTitleEnabled(false)

        val startDate = LocalDate()

        val monthNameTextView = findViewById(R.id.month_name_text_view) as CustomTextView
        monthNameTextView.text = startDate.monthOfYear().asText

        mClassDurationButton = findViewById(R.id.class_duration_button) as (CustomButton)
        ...
        val calendarView = findViewById(R.id.personal_calendar_view) as (LightCalendarView)
        ...
        calendarView.setOnStateUpdatedListener(object : LightCalendarView.OnStateUpdatedListener {
            ...
            override fun onDateSelected(date: Date) {
                ...
                if (isSelectedDatePresentOrPast(date)) {
                    ...
                    val alertDialog = AlertDialog.Builder(this@SearchSpecificPersonalActivity)
                    alertDialog.setIcon(getDrawable(R.drawable.ic_log_out_24dp))
                            .setTitle(getString(R.string.date_unavailable))
                            .setMessage(R.string.correct_date_instruction)
                            .show()

                    val fragmentTransaction = supportFragmentManager.beginTransaction()
                    val dayAgendaFragment = supportFragmentManager.findFragmentByTag("dayAgendaFragment")
                    ...
                    if (!mSelectedDateCode.equals(selectedDateCode)) {
                        ...
                        val fragmentTransaction = supportFragmentManager.beginTransaction()
                        var dayAgendaFragment = supportFragmentManager.findFragmentByTag("dayAgendaFragment")
    ...
    private fun setDefaultProfileImage(){
        val defaultImage = resources.getDrawable(R.drawable.head_placeholder)
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        ...
        if (item != null) {
            ...
            if (searchView != null) {
                val compName = ComponentName(this, SearchSpecificPersonalActivity::class.java)
                            ...
                            mDatabaseReference.child(Constants.FIREBASE_LOCATION_PERSONAL_TRAINER).addListenerForSingleValueEvent(object : ValueEventListener {
                                ...
                                override fun onDataChange(dataSnapshot: DataSnapshot?) {
                                    ...
                                    if (dataSnapshot != null && dataSnapshot.exists()) {
                                        ...
                                        val noResultTextView = findViewById(R.id.no_results_text_view) as TextView
                                        val view = findViewById(R.id.personal_view)
    ...
    private fun getPersonalWorkingLocations(personal: PersonalTrainer){
        mDatabaseReference.child(Constants.FIREBASE_LOCATION_GYMS).child(Utils.encodeEmail(personal.email))
                .addListenerForSingleValueEvent(object : ValueEventListener{
                    override fun onCancelled(p0: DatabaseError?) {
                        Utils.generateWarningToast(this@SearchSpecificPersonalActivity, "Fetch working places failed").show()
                    }
    ...
    private fun getPersonalSpecialties(personal: PersonalTrainer){
        mDatabaseReference.child(Constants.FIREBASE_LOCATION_SPECIALTIES).child(Utils.encodeEmail(personal.email)).child("specialties")
                .addListenerForSingleValueEvent(object : ValueEventListener{
                    override fun onCancelled(p0: DatabaseError?) {
                        Utils.generateWarningToast(this@SearchSpecificPersonalActivity, "Fetch specialties failed").show()
                    }
    ...
    private fun filterResults(query: String, dataSnapshot: DataSnapshot): PersonalTrainer? {
        ...
        when {
            ...
            else -> personals.filter { it.name.toUpperCase().contains(query.toUpperCase()) }.forEach { return it }
        }

        return null
    }

    private fun configurePersonalView(personal: PersonalTrainer) {

        val imageCatcher = FirebaseImageCatcher(this)
        imageCatcher.getPersonalProfilePicture(this, Utils.encodeEmail(personal.email))

        val nameTextView = findViewById(R.id.personal_name_text_view) as TextView
        val gradeTextView = findViewById(R.id.grade_text_view) as TextView
    ...
    override fun setDuration(duration: Int) {
        val textDuration = Utils.getClassDurationText(this, duration)
    ...
    override fun onProfileImageReady(personProfileImage: ByteArray?) {
        val profileImageView = findViewById(R.id.profile_image_view) as ImageView
